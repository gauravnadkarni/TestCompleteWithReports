...
var exeStartTime,exeEndTime,tcStartTime,tcEndTime,strHTMLHighLevelReport;

//set ProjectSuite Varaibles
  ...
  if (intCount === 0 || aqString.GetLength(intCount)== 1 ){
	...
	switch(strStatus)
	{
		...
		case "Fail":
			...
			break;	
    case "total":
			tempCode = "  Total Test Step(s)  " + "<style type=" + chr(34) + "text/css" + chr(34) + ">" + ".totalCircle {    display:inline-block;    border-radius:50%;    border:2px solid;  border-color: Blue; font-size:36px;}"
...
/*function closeBrowser(){
  ...
  for(var i in arr)  {
    while (Sys.WaitBrowser(arr[i], 500).Exists){
...
function closeBrowser()
{
let bArray = ["iexplore", "firefox","msedge","chrome"]; 
 for (let i = 0; i < bArray.length; i++)
{
   let b = Sys.FindChild("ProcessName", bArray[i]);
   while (b.Exists)
    {
      b.Close();
      // Checks whether the browser is closed
      if (b.Exists)
      {
        Log.Warning(b.ProcessName + " seems to hang, terminating.");
        // Terminates the browser
        b.Terminate();
      }
      // Obtains another instance of the browser
        b = Sys.FindChild("ProcessName", bArray[i]);
    }
}   
}

function closeAllBrowser(){
  var arr = Array("iexplore", "chrome","msedge");
  for(var i in arr)  {
...
function closeBrowserSpecific(browserName){
    while (Sys.WaitBrowser(browserName, 500).Exists){
    var b = Sys.Browser(browserName);
    if (b.Exists && b.ChildCount>0 ){
      var page = b.Page("*");
      if (page.Exists) page.Close();  
    }
...
function openNewIEBrowser(url){
  ...
  //Function to launch new Edge browser
function openNewEdgeBrowser(url){
  openNewBrowser(btEdge, url);
  }
...
// Function to open new tab in IE
function OpenNewTab(URL)
{
  var browser = Sys.Browser();
  if ((browser.ProcessName == "iexplore"))
